...
success <- data.frame(HighSBodden=round(1-length(summary[summary$clust=="High saline|warm"&summary$lda.error==1,]$ID)/length(summary[summary$clust=="High saline|warm",]$ID),2), 
                      ...
                      Overall=round(1-length(summary[summary$lda.error==1,]$ID)/length(summary$ID),2))
```
...
```{r dtw graphics, include=FALSE, eval=FALSE}
...
A <- ggplot(Bgroup.clus[Bgroup.clus$clust2=="High saline|warm",])+
  geom_line(aes(dist, Sr*1000, group=ID), color="grey", size=.5, alpha=.8)+
  ...
  theme(legend.position = "none",
        ...
        plot.margin = margin(c(1,1,-14.5,1)),
        axis.text.x = element_text(color = "white"),
        axis.text.y = element_text(color = "black", face = "bold", size = 8),
        axis.ticks.x = element_line(colour = "transparent"),
        axis.title.y = element_text(size = 8))+
  annotate("text", x=1050, y=7.4, label="High S, thermal constant (N=35)", 
           size=1.5, hjust=0.5, fontface="bold")
...
B <- ggplot(Bgroup.clus[Bgroup.clus$clust2=="Int saline|cold",])+
  ...
  theme(legend.position = "none",
        ...
        panel.border = element_rect(fill = NA, size = 2),
        plot.margin = margin(c(1,1,-14.5,-5)),
        axis.text.x = element_text(color = "white"),
        axis.ticks.x = element_line(colour = "transparent"))+
  annotate("text", x=1050, y=7.4, label="Int. S, thermal cold (N=15)", 
           size=1.5, hjust=0.5, fontface="bold")
...
C <- ggplot(Bgroup.clus[Bgroup.clus$clust2=="Low saline|cold",])+
  ...
  theme(legend.position = "none",
        ...
        axis.text.x = element_text(color = "white"),
        axis.ticks.x = element_line(colour = "transparent"))+
  annotate("text", x=1050, y=7.4, label="Low S, thermal shift (N=19)", 
           size=1.5, hjust=0.5, fontface="bold")
...
D <- ggplot(Fgroup.clus[Fgroup.clus$clust2=="FW high saline|shift",])+
  ...
  theme(legend.position = "none",
        ...
        axis.text.x = element_text(color = "white"),
        axis.ticks.x = element_line(colour = "transparent"))+
  annotate("text", x=1050, y=7.4, label="FW high S, thermal shift (N=9)", 
           size=1.5, hjust=0.5, fontface="bold")
...
E <- ggplot(Fgroup.clus[Fgroup.clus$clust2=="FW low saline|cold",])+
  ...
  theme(legend.position = "none",
        ...
        axis.text.x = element_text(color = "white"),
        axis.ticks.x = element_line(colour = "transparent"))+
  annotate("text", x=1050, y=7.4, label="FW resident, thermal cold (N=4)", 
           size=1.5, hjust=0.5, fontface="bold")
...
F1 <- ggplot(Fgroup.clus[Fgroup.clus$clust2=="FW low saline|shift",])+
  ...
  theme(legend.position = "none",
        ...
        axis.text.x = element_text(color = "white"),
        axis.ticks.x = element_line(colour = "transparent"))+
  annotate("text", x=1050, y=7.4, label="FW resident, thermal shift (N=4)", 
           size=1.5, hjust=0.5, fontface="bold")

A1 <- ggplot(Bgroup.clus[Bgroup.clus$clust2=="High saline|warm",])+
  geom_line(aes(dist, d18O_res, group=ID), color="grey", size=.5, alpha=.8)+
  ...
  theme(legend.position = "none",
        panel.grid.major.y = element_line(),
        panel.border = element_rect(fill = NA, size = 2),
        plot.margin = margin(c(-14.5,1,1,1)),
        axis.text.x = element_text(color = "black", face = "bold", size = 6),
        axis.text.y = element_text(color = "black", face = "bold", size = 6),
...
B1 <- ggplot(Bgroup.clus[Bgroup.clus$clust2=="Int saline|cold",])+
  ...
  theme(legend.position = "none",
        ...
        axis.title.y = element_blank(),
        panel.border = element_rect(fill = NA, size = 2),
        plot.margin = margin(c(-14.5,1,1,-5)),
        axis.text.x = element_text(color = "black", face = "bold", size = 6))
...
Plot <- ggarrange(A,B,C,D,E,F1,A1,B1,C1,D1,E1,F2, 
          nrow = 2, ncol = 6, 
...
## Figure main effects
```{r temp & clust effect figure, eval=FALSE}
...
#rename clusters
pikes3 <- pikes2%>%
  mutate(clust2=case_when(clust=="F_HighS"~2,
                          clust=="F_IntS"~2,
                          clust=="F_LowS"~1,
                          clust=="S_HighS"~4,
                          clust=="S_IntS"~3,
...
pikes3$clust <- factor(pikes3$clust2,
                       levels = c(1,2,3,4),
                       labels = c("Freshwater resident",
                                  "Low salinity/thermal shift",
                                  "Saline shift/thermal shift", 
                                  "High salinity/stable temperature"))
...
#Cluster effect
Clust <- ggplot(pikes3[pikes$lifeyear<=7,], aes(factor(lifeyear), log(inc), color=clust))+
  ...
  scale_color_manual(breaks = c("Freshwater resident",
                                "Low salinity/thermal shift",
                                  "Saline shift/thermal shift", 
                                  "High salinity/stable temperature"),
                     values = mypallette)+
...
pikedata <- pikedata %>% filter(area != "KD") %>% filter(id != "BH-01908")

# Cleanup
growthdata <- pikedata %>%
  transmute(id = as.integer(substr(pikedata$id,4,9)),
            sex = sex,
            weight = weight,
            TL = TL,
            radmm = back_length/10,
            date = date,
            area = area,
            age = age,
            agei = agei)
...
IDfemale <- growthdata %>% filter(sex==0)%>%
  ...
  arrange(id)%>%
  distinct()%>%
  mutate(ID2=seq(1,nrow(IDfemale),1))

IDmale <- growthdata %>% filter(sex==1)%>%
  transmute(id=id)%>%
  arrange(id)%>%
  distinct()%>%
  mutate(ID2=seq(1,nrow(IDmale),1))
...
standata_growth = list(
  Radmm = growthdata$rad,
  Agei = growthdata$agei,
  Sexbin = growthdata$sex,
  N = nrow(growthdata),
  mu_linf = 150,
  sig_linf = 10,
  n_groupmax = as.integer(max(growthdata[growthdata$sex==0,]$ID2)),
  n_group1 = as.integer(max(growthdata[growthdata$sex==0,]$ID2)),
  n_group2 = as.integer(max(growthdata[growthdata$sex==1,]$ID2)),
  group_id = growthdata$ID2
)
```

```{r individual Growth analysis, include=FALSE}
# Script to fit individual-level growth curve
stancode <- ("Data/Stancode/Growth_model_hierarchical3.stan")

fit = stan(stancode, iter = 40000, data = standata_growth,
           control = list(adapt_delta = 0.9, max_treedepth = 10))

save(fit, file = "Data/Pike/Fit_individual_GM.RData")

print(fit, pars = c("LInf1", "LInf2", "tZero1", "tZero2", "k1_mu", "k2_mu", "k1_sig", "k2_sig", "phi" ))

samples <- as.matrix(fit, pars=c("LInf", "LInf1", "k", "k1", "tZero", "tZero1"))
head(samples)
...
Pred1 <- t(apply(samples[, c(1, 3, 7)], 1, Incpredict, Agei = 1:20))
Pred2 <- t(apply(samples[, c(2, 5, 8)], 1, Incpredict, Agei = 1:20))

Predtotal <- rbind(Pred1, Pred2)

#Make quantiles
Quantile1 <- data.frame(t(apply(Pred1, 2, quantile, c(0.5, 0.05, 0.95))))
colnames(Quantile1) <- c("median", "lower", "upper")
Quantile1$Agei <- c(1:20)
Quantile2 <- data.frame(t(apply(Pred2, 2, quantile, c(0.5, 0.05, 0.95))))
colnames(Quantile2) <- c("median", "lower", "upper")
Quantile2$Agei <- c(1:20)

Quantileplot <- ggplot() +
  geom_ribbon(data = Quantile1, aes(x = Agei, ymin = lower, ymax = upper), 
              alpha = 0.3, fill = "blue")+
  geom_line(data = Quantile1, aes(x = Agei, y = median), color = "blue")+
  geom_ribbon(data = Quantile2, aes(x = Agei, ymin = lower, ymax = upper), 
              alpha = 0.3, fill = "red")+
  geom_line(data = Quantile2, aes(x = Agei, y = median), color = "red")+
  geom_point(data = growthdata[growthdata$sex==0,], 
             aes(x = age, y = radmm), color="blue")+
  geom_point(data = growthdata[growthdata$sex==1,], 
             aes(x = age, y = radmm), color="red")+
  theme_minimal()
