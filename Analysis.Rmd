...
priors <- read.delim("Data/Priors_pike.csv", sep = ";", 
                     header = T, stringsAsFactors = F)
...
IDpikef <- unique(growthdata[growthdata$sex==0,]$id)

f <- NULL

for (i in 1:length(IDpikef)) {
 d <- ifelse(growthdata[which(growthdata$id==IDpikef[i]),"sex"]==0,i,NA)
 f <- c(f,d)
}

IDpikem <- unique(growthdata[growthdata$sex==1,]$id)

m <- NULL

for (i in 1:length(IDpikem)) {
 b <- ifelse(growthdata[which(growthdata$id==IDpikem[i]),"sex"]==1,i,NA)
 m <- c(m,b)
}

growthdata$groupID <- NA
growthdata[growthdata$sex==0,]$groupID <- f
growthdata[growthdata$sex==1,]$groupID <- m

standata_growth = list(
  Lengthi = growthdata$Length_at_age/10,
  Agei = growthdata$agei,
  Sexbin = growthdata$sex,
  N = nrow(growthdata),
  alinf = 150^2/5^2,
  blinf = 150/5^2,
  n_groupmax = max(growthdata$groupID),
  n_group = max(growthdata$groupID[growthdata$sex==0]),
  n_group1 = max(growthdata$groupID[growthdata$sex==1]),
  group_id = growthdata$groupID# smth wrong here
)

length(unique(growthdata$id))
```

```{r individual Growth analysis, include=FALSE}
# Script to fit chosen growth curve to age data


stancode <- ("Growth_model_hierarchical3.stan")

fit = stan(stancode, iter = 40000, data = standata_growth,
           control = list(adapt_delta = 0.9, max_treedepth = 10))

save(fit, file = "Data/Fit_individual_GM")

print(fit, pars = c("LInf", "LInf1", "tZero", "tZero1", "k_mu", "k1_mu", "k_sig", "k1_sig", "phi" ))
