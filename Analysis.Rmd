...
#                          header = T, dec = ".", sep = "\t",
#                          stringsAsFactors = F)
oto_lowres <- read.delim("Data/Pike/Oto_LA-to-d18O_interpolation_KD.txt",
                         header = T, dec = ".", sep = "\t",
...
Elemeans <- oto_lowres%>%
  ...
  na.omit()%>%
  summarise(ID=ID,
            area=case_when(area=="Marine"~"Lagoon",
                           area=="Ana"~"Tributary",
                           area=="Freshwater"~"Tributary",
...
fit2 <- Mclust(Elemeans[,c(3,8)], G=3, modelNames = "VVI")
