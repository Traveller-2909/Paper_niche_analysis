...
dataB1 <- interpol(series = Bgroup1, length = 40, element = "Sr")
dataB2 <- interpol(series = Bgroup1, length = 40, element = "d18O_res")
...
#Plot best solution lagoons
# png(filename = "C:/Users/timor/Nextcloud2/Timo/BODDENHECHT/Chapter 2/Stats/Niche analysis/Figures/Year_3_Sr_d18O_centroids.jpg", width = 2400, height = 800)
plot(clust_B$k_3, type="centroid", linetype="solid", 
     alpha=0.5, cex=1)+
  ylim(-2.5,3)+ggtitle(expression("Sr/"~delta^18~O~"cluster centroids"))+
  labs(x="transect cell", y="scaled value")+
  theme(axis.text = element_text(size = 25, color="black"),
...
mypallette <- viridis::viridis(5, direction = 1)
...
D <- ggplot(Fgroup.clus[Fgroup.clus$clust2=="FW high saline|warm",])+
  geom_line(aes(dist, Sr*1000, group=ID), color="grey", size=.5, alpha=.8)+
  ...
  theme(legend.position = "none",
        ...
        axis.text.x = element_text(color = "white"),
        axis.ticks.x = element_line(colour = "transparent"))+
  annotate("text", x=1050, y=7.4, label="FW high S, thermal shift (N=13)", 
           size=1.8, hjust=0.5, fontface="bold")
...
D1 <- ggplot(Fgroup.clus[Fgroup.clus$clust2=="FW high saline|warm",])+
  geom_line(aes(dist, d18O_res, group=ID), color="grey", size=.5, alpha=.8)+
...
Plot <- ggarrange(A,B,C,D,E,A1,B1,C1,D1,E1, 
          nrow = 2, ncol = 5, 
          widths = c(1,1,1,1,1),
          heights = c(1.5,1.5),
          align = "v",
...
png(filename = "Figures/Life-history-plots.png", 
...
pikedat <- allpike%>%
  transmute(ID=ID,
            ...
            clust=case_when(clust2 == "High saline|warm"~"HighS|Tconstant",
                            ...
                            clust2 == "FW high saline|warm"~"FWHighS|Tshift",
