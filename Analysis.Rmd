...
```{r DTW clustering 3 year olds, include=FALSE}
...
```{r cluster assignment 3 year olds, include=FALSE}
...
```{r dtw data prep whole sample, include=FALSE}
rm(list = ls())
#read in data
load("Data/Pike/Adults_element_transects.RData")
...
```{r DTW clustering all, include=FALSE}
#cluster Bodden with cutoff test
clust_B <- tsclust(series = Bseries, type = "hierarchical",
                  k = 2L:6L,centroid = dba,
                  control = hierarchical_control(method = "ward.D"),
...
```{r cluster assignment all, include=FALSE}
#assign clusters to fish
fishassign_B2 <- data.frame(ID=names(Bseries), 
...
```{r join growth measurements with cluster, include=FALSE}
...
Temp <- ggplot(pikes2, aes(res_mean, log(inc), color=agefact2))+
  ...
  labs(x=expression(bold("scaled"~delta^18*"O"~(x%*%"standard deviation"))), 
