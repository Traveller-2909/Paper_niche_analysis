...
init_fun = function() {
	list(
		LInf_i_raw = runif(86, -0.2, 0.2),
		tZero_i_raw = runif(86, -0.2, 0.2),
		k_i_raw = runif(86, -0.2, 0.2),
		LInf_eco_raw = runif(6, -0.2, 0.2),
		k_eco_raw = runif(6, -0.2, 0.2),
		tZero_eco_raw = runif(6, -0.2, 0.2),
		LInf = runif(1, 2, 4),
		LInf_sig = runif(1, 0, 2),
		k = runif(1, 0.05, 0.15),
		k_sig = runif(1, 0, 0.1),
		tZero = runif(1, -1.5, -0.1),
		tZero_sig =  runif(1, 0, 0.5),
		LInf_eco_sig = runif(6, 0.2, 2),
		k_eco_sig = runif(6, 0, 0.1),
		tZero_eco_sig = runif(6, 0.2, 2),
		phi = runif(1, 0, 0.1)
	)
}
```
...
stancode <- ("Matt input/revised/growth_model_revised.stan")
mod = stan_model(stancode, model_name = "Individual pike growth")
fit = sampling(mod, iter = 20000, warmup = 10000, data = standata_growth, init = init_fun,
			   control = list(adapt_delta = 0.995, max_treedepth = 20))
saveRDS(fit, "Matt input/revised/revised_fit.rds")

print(fit, pars = c("LInf_eco", "k_eco", "tZero_eco"))
```
...
mylabel <- c("Brackish resident","Freshwater resident","Anadromous","Cross-habitat")
...
ecotype_curves = function(x, dat) {
	...
	for(i in 1:4) {
		pl_curves[[i]] = ggplot(data = eco_plot[eco_id == i]) + 
			...
			geom_line(aes(x = Agei, y = median), color = mypallette[i], size = 2) + 
		  ggtitle(mylabel[i])+
			geom_line(data = fish_plot[eco_id == i], aes(x = Agei, y = fit, group = fish_id), color = mypallette[i], size = 0.5, alpha = 0.3) + 
			geom_point(data = dat[dat$ecotype_num == i, ], 
					   aes(x = lifeyear, y = rad_mm, group = ID), size = 1, alpha = .9, color="black") + 
		  ylim(0,3.5)+scale_x_continuous(breaks = c(1:15), labels = c(1:15))+
...
png(filename = "Figures/Individual_growth_revised.png", width = 5000, height = 5000, res = 600)
ecotype_curves(fit, growthdata)
dev.off()
```
...
ggplot(new_df, aes(x = cluster, y = value, color = ecotype)) +
  geom_count() +
  labs(title = "Frequency of Cluster Values by Ecotype",
       x = "Cluster",
       y = "Value")
...
df_F <- subset(df_long, ecotype == "FW_resident")
df_A <- subset(df_long, ecotype == "Anadromous")
df_T <- subset(df_long, ecotype == "Trans-habitat")
df_B <- subset(df_long, ecotype == "BW-resident")
...
FW <- ggplot(df_F, aes(x = cluster, y = prob.x, fill = cluster)) +
  ...
  labs(title = "Freshwater resident ecotype", x = "Cluster", y = "Proportion")+
  ylim(0,1)+
...
Ana <- ggplot(df_A, aes(x = cluster, y = prob.x, fill = cluster)) +
  geom_boxplot() +
  stat_summary(fun=mean, geom="point", shape=18, size=5, color="red", fill="red")+
  labs(title = "Anadromous ecotype", x = "Cluster", y = "Proportion") +
  ylim(0,1)+
  scale_fill_manual(values = c("#00BFB2", "#C5D86D", "#F1C40F", "#004643"))+
  theme_bw()+
  pl_theme
...
Cross <- ggplot(df_T, aes(x = cluster, y = prob.x, fill = cluster)) +
  geom_boxplot() +
  stat_summary(fun=mean, geom="point", shape=18, size=5, color="red", fill="red")+
  labs(title = "Cross-habitat ecotype", x = "Cluster", y = "Proportion") +
  ylim(0,1)+
  scale_fill_manual(values = c("#00BFB2", "#C5D86D", "#F1C40F", "#004643"))+
  theme_bw()+
  pl_theme
...
BW <- ggplot(df_B, aes(x = cluster, y = prob.x, fill = cluster)) +
  geom_boxplot() +
  stat_summary(fun=mean, geom="point", shape=18, size=5, color="red", fill="red")+
  labs(title = "Brackish resident ecotype", x = "Cluster", y = "Proportion") +
  ylim(0,1)+
  scale_fill_manual(values = c("#00BFB2", "#C5D86D", "#F1C40F", "#004643"))+
  theme_bw()+
  pl_theme
  
png(filename = "Figures/Ecotypes_genetic_clusters.png", width = 5000, height = 5000, res = 600)
grid.arrange(FW, Ana, Cross, BW)
dev.off()
```
