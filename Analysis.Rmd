...
Carbon <- ggplot(PikeSI, aes(d15N, d13C, color=clust))+geom_point()+theme_minimal()+
  stat_ellipse(aes(fill = clust, color = clust), alpha = 0.2, level = 0.95, geom = "polygon")+
...
Sulphur <- ggplot(PikeSI, aes(d15N, d34S, color=clust))+geom_point()+theme_minimal()+
  stat_ellipse(aes(fill = clust, color = clust), alpha = 0.2, level = 0.95, geom = "polygon")+
  scale_color_viridis_d(direction = -1)+
  scale_fill_viridis_d(direction = -1)+
  theme(panel.background = element_blank(), panel.grid.major = element_line(colour = "grey", size = 0.5),
