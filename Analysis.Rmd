...
A <- ggplot(Bgroup.clus[Bgroup.clus$clust2=="High saline|warm",])+
  ...
  coord_cartesian(clip = "off")+
  annotation_custom(grob = text_grob(label = "High S, thermal constant (N=35)",
                                     face = "bold", size = 8),
...
standata_growth = list(
  ...
  tZero_mu = mean(priors$to),
  tZero_sig = sd(priors$to),
  ecotype = as.integer(growthdata$ecotype),
  N = nrow(growthdata),
  n_groupmax = as.integer(max(growthdata$id2)),
  n_group1 = as.integer(max(growthdata[growthdata$ecotype==1,]$id2)),
  n_group2 = as.integer(max(growthdata[growthdata$ecotype==2,]$id2)),
  n_group3 = as.integer(max(growthdata[growthdata$ecotype==3,]$id2)),
  n_group4 = as.integer(max(growthdata[growthdata$ecotype==4,]$id2)),
  n_group5 = as.integer(max(growthdata[growthdata$ecotype==5,]$id2)),
  n_group6 = as.integer(max(growthdata[growthdata$ecotype==6,]$id2)),
...
stancode <- ("Data/Stancode/Growth_model_hierarchical4_ecotypes.stan")
...
fit = stan(stancode4, iter = 1000, data = standata_growth_2, 
           model_name = "Individual pike growth",
           control = list(adapt_delta = 0.9, 
                          max_treedepth = 12))

fit_large = stan(stancode4, iter = 5000, data = standata_growth_2, 
           model_name = "Individual pike growth",
           control = list(adapt_delta = 0.9, 
                          max_treedepth = 12))
...
#Pairs plots
pairs(fit, pars = c("LInf","k","tZero"))
pairs(fit, pars = c("LInf_eco","k_eco"))
...
Pred1 <- t(apply(samples[, c(1, 7, 13)], 1, Incpredict, Agei = 1:20))
Pred2 <- t(apply(samples[, c(2, 8, 14)], 1, Incpredict, Agei = 1:20))
Pred3 <- t(apply(samples[, c(3, 9, 15)], 1, Incpredict, Agei = 1:20))
Pred4 <- t(apply(samples[, c(4, 10, 16)], 1, Incpredict, Agei = 1:20))
Pred5 <- t(apply(samples[, c(5, 11, 17)], 1, Incpredict, Agei = 1:20))
Pred6 <- t(apply(samples[, c(6, 12, 18)], 1, Incpredict, Agei = 1:20))
...
Quantile3$Agei <- c(1:20)

Quantile4 <- data.frame(t(apply(Pred4, 2, quantile, c(0.5, 0.05, 0.95))))
colnames(Quantile4) <- c("median", "lower", "upper")
Quantile4$Agei <- c(1:20)

Quantile5 <- data.frame(t(apply(Pred5, 2, quantile, c(0.5, 0.05, 0.95))))
colnames(Quantile5) <- c("median", "lower", "upper")
Quantile5$Agei <- c(1:20)

Quantile6 <- data.frame(t(apply(Pred6, 2, quantile, c(0.5, 0.05, 0.95))))
colnames(Quantile6) <- c("median", "lower", "upper")
Quantile6$Agei <- c(1:20)

mypallette <- viridis::viridis(6, direction = 1)

Ecocurves <- ggplot() +
  geom_ribbon(data = Quantile1, aes(x = Agei, ymin = lower, ymax = upper), 
              alpha = 0.3, fill = mypallette[1])+
  geom_line(data = Quantile1, aes(x = Agei, y = median), color = mypallette[1])+
  geom_ribbon(data = Quantile2, aes(x = Agei, ymin = lower, ymax = upper), 
              alpha = 0.3, fill = mypallette[2])+
  geom_line(data = Quantile2, aes(x = Agei, y = median), color = mypallette[2])+
  geom_ribbon(data = Quantile3, aes(x = Agei, ymin = lower, ymax = upper), 
              alpha = 0.3, fill = mypallette[3])+
  geom_line(data = Quantile3, aes(x = Agei, y = median), color = mypallette[3])+
  geom_ribbon(data = Quantile4, aes(x = Agei, ymin = lower, ymax = upper), 
              alpha = 0.3, fill = mypallette[4])+
  geom_line(data = Quantile4, aes(x = Agei, y = median), color = mypallette[4])+
  geom_ribbon(data = Quantile5, aes(x = Agei, ymin = lower, ymax = upper), 
              alpha = 0.3, fill = mypallette[5])+
  geom_line(data = Quantile5, aes(x = Agei, y = median), color = mypallette[5])+
  geom_ribbon(data = Quantile6, aes(x = Agei, ymin = lower, ymax = upper), 
              alpha = 0.3, fill = mypallette[6])+
  geom_line(data = Quantile6, aes(x = Agei, y = median), color = mypallette[6])+
  geom_point(data = growthdata, 
             aes(x = agei, y = radmm), size = 0.5)+
  labs(x="Age", y="otolith increment (mm)")+
  theme_minimal()

png(filename = "Figures/Ecotypes_VB_curves.png", 
    width = 3000, height = 1500, res = 600)
Ecocurves
dev.off()
```
