...
"#004643" --> brackish resident,"#00BFB2" --> anadromous,"#F1C40F" --> cross-habitat,"#C5D86D" --> frehwater resident,
"#AF3800" --> unknown
...
write.table(pike_inc2, file = "Data/Pike/Oto_increment_data_with_rad.txt",
            sep = "\t", dec = ".", row.names = F)
```
...
plot(pikes2$resid~pikes2$ecotype, ylab = "Residuals", xlab = "Cluster", las = 2)
...
## Figure main effects
```{r temperature effect figure, eval=FALSE}
...
#Temperature effect
Temp <- ggplot(pikes2, aes(res_mean, log(inc_um), color=as.factor(lifeyear)))+
  geom_point(size=.5)+
...
Sal <- ggplot(pikes2, aes(Sr_mean, log(inc_um), color=agefact2))+
  geom_point(size=.5)+
  stat_smooth(method = "lm", fill="gray", fullrange = T, lwd=.8)+
  labs(x=expression(bold("scaled"~"Sr/Ca"~("z-score"))), 
       y=expression(bold("log increment"~(mu*"m"))),
       color="ageclass")+
  scale_color_viridis_d()+
  theme_classic()+
  theme(panel.grid.major = element_line(),
        ...
        legend.key.size = unit(.8,"line"))+
  coord_cartesian(clip = "off")+
  geom_segment(x=0, y=3.6, xend=2, yend=3.6, 
               lineend = "butt", linejoin = "mitre",
               size=2, arrow = arrow(length = unit(0.18, "inches")),
               color="#FF6666")+
  annotation_custom(grob = text_grob(label = "higher salinity", 
                                     rot = 0, face = "bold", size = 8),
                                     ymin=3.58, ymax=3.58,
                                     xmin=1, xmax=2)+
  geom_segment(x=0, y=3.6, xend=-2, yend=3.6, 
               lineend = "butt", linejoin = "mitre",
...
png(filename = "Figures/Salinity_effects_growth.png", 
    width = 3000, height = 1500, res = 600)
annotate_figure(Sal, top = text_grob("Effect of lifelong salinity on pike growth", color = "black", face = "bold", size = 10, vjust = 1, hjust = 0.7))
dev.off()
```
...
Newdata$prediction <- 10^(predict(pikemodel.final, newdata = Newdata, allow.new.levels=TRUE))
...
ggplot(Newdata2, aes(factor(lifeyear), prediction, fill=ecotype), alpha=0.6)+
  geom_boxplot(lwd=.1, outlier.shape = NA, width=.5, position = position_dodge(width = NULL), color="black")+
  labs(x="Age (years)", y=expression(bold("predicted increment"~(mu*"m"))),
       color="ecotype")+
...
growthdata <- pikedata %>%
  transmute(ID = ID,
            ...
            age = age,
            lifeyear = lifeyear,
            ecotype=case_when(ecotype=="BW-resident"~1,
                              ecotype=="FW_resident"~2,
                              ecotype=="Anadromous"~3,
                              ecotype=="Trans-habitat"~4))

write.table(growthdata, file = "Data/Pike/Pikedata_Stan.txt", row.names = F, sep = "\t", dec = ".")
...
standata_growth = list(
  ...
  N = nrow(growthdata),
  n_fish = as.integer(length(unique(pikedata$ID))),
  n_eco = as.integer(max(growthdata$ecotype)),
  fish_id = as.integer(growthdata$ID2))
fishtable = unique(growthdata[, c("ID2", "ecotype")])
...
bayesplot::mcmc_hist(as.array(fit, pars = "LInf_eco"))
