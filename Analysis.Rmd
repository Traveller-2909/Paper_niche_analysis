...
```{r pca analysis, include=FALSE, eval=FALSE}
Elemeans2 <- as.matrix(Elemeans[,c(3:8)])

PCA <- prcomp(Elemeans2)

fviz_eig(PCA)

#PCA visual for indviduals
groups <- as.factor(Elemeans$area)
fviz(PCA, "ind", color = groups)

#PCA visual for variables
fviz_pca_var(PCA, col.var = "contrib", #color by contribution
...
allplot <- align_plots(A, B, C, D, A1, B1, C1, D1, align = "hv")
...
F1 <- ggplot(Fgroup.clus[Fgroup.clus$clust2=="FW high saline|const",])+
  ...
  theme(legend.position = "none", 
        ...
        axis.line = element_line(size = 0.3))+
  coord_cartesian(clip = "off")+
  annotation_custom(grob = text_grob(label = "high saline, const (N=11)", size = 7),
                    ymin = 10, ymax = 10, xmin = 750, xmax = 750)
...
png(filename = "Figures/DTW-plots/Allplot_FW_midres.png", width = 6000, height = 2000, res = 600)
...
growthdata <- pikedata %>%
  transmute(ID = ID,
            ...
            ecotype_char=case_when(ecotype=="FW_resident"~"FW-resident",
                                   TRUE~ecotype),
            ecotype_num=case_when(ecotype=="BW-resident"~1,
...
Pred1 <- t(apply(samples[, c(1, 5, 9)], 1, Incpredict, Agei = 1:15))
Pred2 <- t(apply(samples[, c(2, 6, 10)], 1, Incpredict, Agei = 1:15))
Pred3 <- t(apply(samples[, c(3, 7, 11)], 1, Incpredict, Agei = 1:15))
Pred4 <- t(apply(samples[, c(4, 8, 12)], 1, Incpredict, Agei = 1:15))
...
Brackish_VB <- ggplot() +
  ...
  geom_ribbon(data = Quantile4, aes(x = Agei, ymin = lower, ymax = upper, fill = "Low S, thermal shift (N=19)"),
              alpha = 0.3)+
  geom_line(data = Quantile4, aes(x = Agei, y = median, color = "Low S, thermal shift (N=19)"), size=2)+
...
geneticsdata <- read.delim("Data/Pike/Genetics/structureK4_otolith_only.txt", sep = "", dec = ".", header = F, stringsAsFactors = F)
