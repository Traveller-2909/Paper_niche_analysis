...
Data <- Data %>% 
  mutate(highest_cluster = apply(Data[, 2:5], 1, which.max)) %>% 
  mutate(highest_cluster = paste0("cluster", highest_cluster))

Otodata <- read.delim("Data/Pike/Pikedata_model.txt", header = T, stringsAsFactors = F, sep = "\t", dec = ".")
Otoclusters <- Otodata%>%
  transmute(ID=ID,
            clust=clust,
            ecotype=ecotype)%>%
...
hist(Otogen$cluster3)


ggplot(Otogen, aes(highest_cluster, clust))+geom_point(position =position_jitter(width = 0.02))
```
